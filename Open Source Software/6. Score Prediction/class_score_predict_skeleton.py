...
if __name__ == '__main__':
    ...
    x = data[:, 0] # midterm scores
    y = data[:, 1] # final scores
    n = len(x)
    x_mean = np.mean(x) # mean of midterm scores
    y_mean = np.mean(y) # mean of final scores
    Sxy = np.sum((x - x_mean) * (y - y_mean)) # deviation product
    Sxx = np.sum((x - x_mean) ** 2) # variance
    slope = Sxy / Sxx
