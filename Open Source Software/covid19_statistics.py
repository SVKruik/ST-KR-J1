...
for idx, pop in enumerate(n_people):
    ratio = pop / sum_people * 100 if sum_people > 0 else 0
    print('| %s | %d | %.1f |' % (regions[idx], pop, ratio))
print()

# Print new COVID-19 cases by region
print('### Korean COVID-19 New Cases by Region')
print('* Total new cases:', sum_covid)
print() # Print an empty line
print('| Region | New Cases | Ratio (%) | New Cases (per 1M) |')
print('| ------ | --------- | --------- | ------------------ |')
for idx, c in enumerate(n_covid):
    ratio = c / sum_covid * 100 if sum_covid > 0 else 0
    print('| %s | %d | %.1f | %.1f |' % (regions[idx], c, ratio, norm_covid[idx]))
print()
